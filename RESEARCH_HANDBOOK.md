...
### §八補記:八陣圖建制定位——烽燧(遊軍斥候)(2026-09-26)

金絲雀依「一陣一策略」原則**不入八陣**(它不是策略,是偵察系統)。建制定位:
**烽燧・遊軍斥候**——仿《李靖問對》八陣之外的遊軍:不列戰陣、不接敵,
職司預警,情報通報全軍。駐地雲垂陣(實時面板每日運行於該分支日報),
各陣取用方式:完整作戰手冊
`git checkout origin/claude/dazzling-curie-f3xzb8 -- CANARY_PLAYBOOK.md`,
數據管道(六鳥日線+四地週/月斜率)同分支每日自動更新。
各陣測試遷移假設時沿用手冊 §7 的口徑紀律(lag=1、固定閾值、t≥3)。

**烽燧響應規則須預先登記(2026-09-26 定,全軍適用)**:烽燧本身閾值固定不調參,
但若各陣事後才決定「這次亮燈我怎樣處理」,調參自由度就從訊號端溜到反應端,
口徑紀律等於被繞過。故**任何陣在開始取用烽火之前**,必須在自己分支記一行
「烽燧響應規則 + 登記日期」,之後只按登記行事;改規則本身也要重新登記並留痕。
未登記的陣視為「未接入烽燧」,不得臨場引用烽火作決策依據。

已登記響應規則:
- **蛇蟠(2026-09-26)**:不響應——照常執行,烽火狀態僅記入 journal/log.csv notes
  供事後歸因。理由:蛇蟠獲利前 10 筆佔 129%,絕不跳單是鐵律,任何臨場過濾都摧毀
  期望值;蛇蟠唯一的停手條件仍是自身滑價紅燈(近 20 筆均值 >5 點)。
