...
def pool_all() -> None:
    ...
    out = {}
    for k in KEYS:
        real = tstat(pooled([R[mk]["abn"][k] for mk in MARKETS]))
        n = min(len(RN[mk][k]) for mk in MARKETS)
        rt = sorted(tstat(pooled([RN[mk][k][i] for mk in MARKETS])) for i in range(n))
        pct = sum(x < real for x in rt) / n if n else float("nan")
        cells = {mk: R[mk]["cells"][k] for mk in MARKETS}
        ...
        if real >= 2.0 and pos_alpha >= 2 and abs_pos and pct >= 0.95:
            v = "✅ 樣本外確認"
            ...
            v = "不確定"
        if k != "H5a" and v.startswith("✅"):
            v += "（次要候選：只標「值得再驗證」，不取代主要候選）"
        out[k] = {"pooled_t": real, "random_pctl": pct, "random_median": rt[n // 2] if n else None,
                  ...
                  "verdict": v, "per_market": {mk: {x: cells[mk][x] for x in ("alpha_t", "cagr", "cagr_etf", "cum", "cum_etf",
                                               ...
                                               for mk in MARKETS}}
        print(f"{k:10} 合併 alpha t {real:5.2f}  隨機第 {pct:.0%} 百分位（中位 {out[k]['random_median']:.2f}）  "
