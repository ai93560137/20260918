...
現況一句話：
  ...
  第二輪「模板股全部等權月換倉＋大市過濾」🔍（合併 4.59、相對等權 3.21、十條全過）——可進衛星倉前向記錄（上限 40 隨機抽版本；港新加印澳先做；證偽寫在 TT_MOMENTUM_BACKTEST.md 第四部分）。港日美數據已用 13 次。
...
待我決定的方向（先問我，不要自行開始）：
  ...
  A2. 把「趨勢模板股全部等權」的月底名單加進每日報告（每月最後一個交易日列名單＋大市過濾狀態），開始前向記錄（改 daily_topdown.py 要推回 gifted-carson）
