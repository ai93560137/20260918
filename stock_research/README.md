...
- **數據斷點**（一日 > +100%／< −60% 的縫接、垃圾列）結構檢查抓不到，動量／排序類策略必須剔除（`tt_momentum_backtest.py` 的 `bad`）；**基準 ETF 也會錯**（0050.TW 拆股回溯只做一半、NIFTYBEES 少一個零）→ `universes/full/etf_fixes.csv`；任何新市場先掃 ETF 一日變動 > 25%
