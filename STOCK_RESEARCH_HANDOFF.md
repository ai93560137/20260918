...
（合併 alpha t 3.17、3.39），但效果大多來自「趨勢模板＋大市過濾＋止損紀律」、依賴少數大贏家、2015 後轉弱。
...
- **保留**：隨機趨勢模板股同一套出場中位數 t 也有 ~2.1（VCP 形態只加一點）；前 10 筆佔獲利 45–81%（絕不可跳單）；
...
3. **「紀律本身」的假設**：隨機趨勢模板股＋同一套大市過濾／止損出場也有效（中位 t ~2.1）——這是看過結果後才發現的，
   要測必須預先登記，並直接在 9 個市場一次測
