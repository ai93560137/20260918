#!/usr/bin/env python3
"""一鍵取得外匯歷史數據（Dukascopy M1／H1／D1 + Yahoo／FRED 第二來源；不進 git，存 GitHub Release「forex-data」）→ data_forex/<PAIR>/。
...
def fetch(pair: str, root: Path) -> bool:
    ...
    m1 = sorted(d.glob(f"{pair}_M1_*.csv.gz"))
...
def merge_m1(pair: str, root: Path, out: Path) -> None:
    ...
    files = sorted((root / "data_forex" / pair).glob(f"{pair}_M1_*.csv.gz"))
    n = 0
        ...
        for p in files:
            with gzip.open(p, "rt", encoding="utf-8") as f:
                ...
                for line in f:
                    w.write(line)
                    n += 1
    print(f"{pair}：M1 合併 {len(files)} 個年檔、{n:,} 根 → {out}")
