...
- `5333.T` xcheck_history：全量歷史比對 1/252 筆收市差 >1%（最大 18.0%，yahoo.co.jp 月線全段 + 最近 20 日）；例 2010-10-29 我們 1486 vs 對方 1219（-18.0%）
...
- `6472.T` xcheck_history：全量歷史比對 1/252 筆收市差 >1%（最大 2.1%，yahoo.co.jp 月線全段 + 最近 20 日）；例 2012-07-31 我們 207.609 vs 對方 212（+2.1%）
- `6473.T` xcheck_history：全量歷史比對 1/252 筆收市差 >1%（最大 1.4%，yahoo.co.jp 月線全段 + 最近 20 日）；例 2012-07-31 我們 684.305 vs 對方 694（+1.4%）
- `6723.T` xcheck_history：全量歷史比對 1/60 筆收市差 >1%（最大 1.1%，yahoo.co.jp 月線全段 + 最近 20 日）；例 2024-03-29 我們 2640.5 vs 對方 2670.5（+1.1%）
- `6976.T` xcheck_history：全量歷史比對 1/252 筆收市差 >1%（最大 1.3%，yahoo.co.jp 月線全段 + 最近 20 日）；例 2012-07-31 我們 640.5 vs 對方 649（+1.3%）
...
- 已比對 56/264 檔、16,611 筆收市價、不符（>1%）8 筆；第二來源沒有的已下市/改代碼 0 檔（）
- 美股：Nasdaq.com 10 年日線逐日比；日股：Yahoo!ファイナンス 1995 起月線（月底、拆股還原）+ 最近 20 日日線
- 調整方法差異（已解釋、不算不符）0 檔：Yahoo 把分拆/股份交換記成非整數拆股並回溯調整價格（總回報正確），第二來源只調真拆股；比對前按事件日倍數對齊。例：
